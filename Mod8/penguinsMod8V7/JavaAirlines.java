...
public class JavaAirlines {
	...
	public static void makeRes(String[][][] f, ArrayList<Customer> cust, ArrayList<Reservation> reser) {
		...
		if (isCust == false) {
			...
			String[] seats = { "A1", "A2", "H1", "H2", "A3", "A4", "H3", "H4", "D3", "D4", "E3", "E4", 
					"B3", "C3", "F3", "G3", "B4", "C4", "F4", "G4" };

			if (seating("first class", "economy").equals("first class")) {
				suggestSeating(reser, d, seats, 0, 4);
			}			
				...
				if (seat.equals("window"))
					suggestSeating(reser, d, seats, 4, 8);
				else
					suggestSeating(reser, d, seats, 8, 12);
			}
	...
	public static void suggestSeating(ArrayList<Reservation> r, LocalDate d, String[] s, int start, int fin) {
		String seat = "";
		...
		if(!seat.equals(""))
			...
			System.out.println("We are sorry there are no avalible seats here. ");		
	}
